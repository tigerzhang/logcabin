...
StateMachineRocksdb::StateMachineRocksdb(std::shared_ptr<RaftConsensus> consensus,
                           Core::Config& config,
                           Globals& globals)
    : consensus(consensus)
    , globals(globals)
...
bool
StateMachineRocksdb::query(const Query::Request& request,
...
void
StateMachineRocksdb::updateServerStats(Protocol::ServerStats& serverStats) const
{
    std::lock_guard<Core::Mutex> lockGuard(mutex);
...
void
StateMachineRocksdb::wait(uint64_t index) const
{
...
bool
StateMachineRocksdb::waitForResponse(uint64_t logIndex,
...
bool
StateMachineRocksdb::isTakingSnapshot() const
{
    std::lock_guard<Core::Mutex> lockGuard(mutex);
    return childPid != 0;
}
...
void
StateMachineRocksdb::startTakingSnapshot()
{
    std::unique_lock<Core::Mutex> lockGuard(mutex);
    if (childPid == 0) {
...
void
StateMachineRocksdb::stopTakingSnapshot()
{
    std::unique_lock<Core::Mutex> lockGuard(mutex);
    int pid = childPid;
    if (pid != 0) {
...
StateMachineRocksdb::getInhibit() const
{
    std::lock_guard<Core::Mutex> lockGuard(mutex);
...
void
StateMachineRocksdb::setInhibit(std::chrono::nanoseconds duration)
{
    std::lock_guard<Core::Mutex> lockGuard(mutex);
...
void
StateMachineRocksdb::apply(const RaftConsensus::Entry& entry)
{
...
void
StateMachineRocksdb::applyThreadMain()
{
    Core::ThreadId::setName("StateMachineRocksdb");
    try {
        while (true) {
...
void
StateMachineRocksdb::serializeSessions(SnapshotStateMachine::Header& header) const
{
...
void
StateMachineRocksdb::expireResponses(Session& session, uint64_t firstOutstandingRPC)
{
...
void
StateMachineRocksdb::expireSessions(uint64_t clusterTime)
{
...
uint16_t
StateMachineRocksdb::getVersion(uint64_t logIndex) const
{
...
void
StateMachineRocksdb::killSnapshotProcess(Core::HoldingMutex holdingMutex,
                                  int signum)
{
    if (childPid != 0) {
...
void
StateMachineRocksdb::loadSessions(const SnapshotStateMachine::Header& header)
{
    sessions.clear();
...
void
StateMachineRocksdb::loadSnapshot(Core::ProtoBuf::InputStream& stream)
{
...
void
StateMachineRocksdb::loadVersionHistory(const SnapshotStateMachine::Header& header)
{
...
void
StateMachineRocksdb::serializeVersionHistory(
...
bool
StateMachineRocksdb::shouldTakeSnapshot(uint64_t lastIncludedIndex) const
{
...
void
StateMachineRocksdb::snapshotThreadMain()
{
    Core::ThreadId::setName("SnapshotStateMachineRocksdb");
    std::unique_lock<Core::Mutex> lockGuard(mutex);
...
void
StateMachineRocksdb::snapshotWatchdogThreadMain()
{
    using Core::StringUtil::toString;
    Core::ThreadId::setName("SnapshotStateMachineRocksdbWatchdog");
    std::unique_lock<Core::Mutex> lockGuard(mutex);
...
void
StateMachineRocksdb::takeSnapshot(uint64_t lastIncludedIndex,
                           std::unique_lock<Core::Mutex>& lockGuard)
{
    ...
    if (pid == -1) { // error
        ...
        // StateMachineRocksdb state comes next
        {
            SnapshotStateMachine::Header header;
...
void
StateMachineRocksdb::warnUnknownRequest(
